...
def write_visuals(args=None):
    ...
    # ---- Robust loader: try normal path, then manual constructor+weights ----
    model = None
    try:
        # Try the library helper first (may work on some versions)
        model = TimeArrowNet.from_folder(
            model_dir,
            from_state_dict=args.update,   # keep your flag
            ignore_commit=True,
        )
        # If that returned a dict by mistake, throw to fallback
        if isinstance(model, dict):
            raise TypeError("from_folder returned a dict (state container); using fallback loader.")
    except Exception as e:
        # Fallback: build the model from kwargs and load weights explicitly
        kwargs_path = model_dir / "model_kwargs.yaml"
        if not kwargs_path.exists():
            raise FileNotFoundError(f"Missing model_kwargs.yaml under {model_dir}") from e
        with open(kwargs_path, "r") as f:
            kwargs = yaml.safe_load(f)

        model = TimeArrowNet(**kwargs)

        # Prefer model_latest.pt, then model.pt, then newest checkpoint
        weight_candidates = [
        ...
        if ckpt_dir.is_dir():
            # grab latest epoch_*.pt if present
            ckpts = sorted(glob.glob(str(ckpt_dir / "epoch_*.pt")))
            if ckpts:
                weight_candidates.append(Path(ckpts[-1]))

        weights_path = next((p for p in weight_candidates if p and p.exists()), None)
        if weights_path is None:
            raise FileNotFoundError(
                f"No weights found in {model_dir}. Looked for model_latest.pt / model.pt / checkpoints/epoch_*.pt"
            )

        state = torch.load(str(weights_path), map_location=args.device)
        # If torch.load returned a wrapper dict, unwrap common keys
    ...
    # Put model on the requested device and set attribute (some code expects model.device)
