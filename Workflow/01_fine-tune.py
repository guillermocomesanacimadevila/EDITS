...
tarrow_path = os.path.abspath(os.path.join(script_dir, '..', 'TAP', 'tarrow'))
if tarrow_path not in sys.path:
    sys.path.insert(0, tarrow_path)
...
def save_full_config(args, outdir):
    ...
    with open(outdir / "train_args_full.yaml", "w") as f:
        yaml.safe_dump({**vars(args), **metadata}, f)
    logger.info(f"Saved full config to {outdir / 'train_args_full.yaml'}")
