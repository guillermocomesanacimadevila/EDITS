...
def main():
    ...
    # parser.add_argument("--cls_head_load_path")
    ...
    parser.add_argument("--is_true_positive", action="store_true",
                        help='automatically evaluated true when this argument is provided')
    ...
    parser.add_argument(
        "--save_data",
        action="store_true",
        help="when this argument is provided, all test datapoints will be saved"
    )
    ...
    # recall the schema of the data: each element contains
    # event_label, time_arrow_label (0: time direction; 1: inverse of time direction),
    # crop_coordinates (of the top left vertex, crop size is 48), predicted label
    ...
    (false_positive_coordinates, false_negative_coordinates,
     ...
     true_negatives_egs, logits_true_negatives) = probing_mistaken_preds(event_rec_model, test_loader, device,
    ...
    if args.save_data:
        print(' - - - saving false positive and false negative datapoints as .pth file  - - - ')
    ...
    num_f_p = len(false_positive_egs)
    num_f_n = len(false_negative_egs)
    imgs_masks_sequences = masks_prep(n_frames=2, masks_path=args.masks_path, delta_frames=1)
    print(f" plotting false positive examples!")
    plot_mistaken_examples(num_egs_to_show=args.num_egs_to_show,
                           total_number=num_f_p,
                           example_set=false_positive_egs,
                           coordinates=false_positive_coordinates,
                           imgs_masks_seq=imgs_masks_sequences,
                           image_outdir=image_output_dir_false_pos)

    print(f"plotting false negative examples!")
    plot_mistaken_examples(num_egs_to_show=args.num_egs_to_show,
                           total_number=num_f_n,
                           example_set=false_negative_egs,
                           coordinates=false_negative_coordinates,
                           imgs_masks_seq=imgs_masks_sequences,
                           image_outdir=image_output_dir_false_neg)
    ...
    if args.is_true_positive:
        ...
        num_t_p = len(true_positives_egs)
        print(f" plotting true positive examples!")
        plot_mistaken_examples(num_egs_to_show=args.num_egs_to_show,
                               total_number=num_t_p,
                               example_set=true_positives_egs,
                               coordinates=true_positives_coordinates,
                               imgs_masks_seq=imgs_masks_sequences,
                               image_outdir=image_output_dir_true_pos)
    ...
    if args.is_true_negative:
        ...
        num_t_n = len(true_negatives_egs)
        print(f" plotting true negative examples!")
        plot_mistaken_examples(num_egs_to_show=args.num_egs_to_show,
                               total_number=num_t_n,
                               example_set=true_negatives_egs,
                               coordinates=true_negatives_coordinates,
                               imgs_masks_seq=imgs_masks_sequences,
                               image_outdir=image_output_dir_true_neg)
