...
def train_cls_head(train_loader, test_loader, patch_size, num_epochs, random_seed, device,
                   model_load_dir, cls_head_arch, TAP_init = 'km_uniform',
    ...
    # define the combined model using TAP and cls_head, the combined model will be trained to perform the
    # event classification task
    combined_model = CellEventClassModel(TAPmodel=model, ClsHead=cls_head)
    ...
    for epoch in range(num_epochs):
        ...
        for datapoint in train_loader:
            ...
            # print(f' x[0][0][0][0][:3] : { x[0][0][0][0][:3]}')
            ...
            # unit test
            # for i, parem in enumerate(combined_model.parameters()):
            #     if i == 0:
            #         print(f'parem : {parem[0]}')
            # set_trace()
            ### end of test
