...
def main():
    ...
    parser.add_argument("--pixel_area_threshold", type=int, help="lower threshold of the pixel area for event labelling")
    ...
    # setting random seed for training data generation
    ...
    image_crops = ConcatDataset(
        (
            _build_dataset(
                ...
                delta_frames=time_delta, #args.delta,
                crops_per_image=args.crops_per_image,
                pixel_area_threshold=args.pixel_area_threshold
            )
            for split in [[0, 1]]  # taking all the frames into consideration
    ...
    image_crops_save_path = os.path.join(args.data_save_dir, 'preprocessed_image_crops.pth')

    torch.save(image_crops_flat, image_crops_save_path)
    print(f"Image crops saved to {image_crops_save_path}!")

    end_time_data_prep = time.time()
    print(f"Time used for data pre-processing : {end_time_data_prep - start_time} seconds")
